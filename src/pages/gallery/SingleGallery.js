    ...
    return (
        <div>
            ...
            {isUserAuthenticated && gallery.buyer_id === null ? (
                <div className="d-lg-flex justify-content-center mt-4 mb-4">
                    {gallery.user_id === activeUser.id ? (
                        ...
                        <button
                            onClick={() =>
                                handleBuyGallery(gallery.id, activeUser.id)
                            }
                        >
                            Buy Gallery
                        </button>
            ...
            {isUserAuthenticated && activeUser.id === gallery.user_id && (
                <>
                    <button onClick={() => handleUpdateGallery(id)}>
                        Update Gallery
                    </button>
                    <>
                        <br />
                    </>
                    <button onClick={() => handleDeleteGallery(id)}>
                        Delete Gallery
                    </button>
                </>
            )}
